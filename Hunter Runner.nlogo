...
to move_malicious_entities
  ...
  [
    ...
    [
      ...
      [
        set distance_moved (speed * comms_broadcast_update)
        check_wall_collision (distance_moved) ; Hunter Runner
      ]
      if (hold_on_target = FALSE)
      [
        forward distance_moved
  ...
  [
    ...
    [
      ...
      [
        ...
        ask ?
        [
          if (breed = targets) ; Need to reduce distance to target to 0.5 at a later time
          [
            set temp_target_type_found target_type
            if(temp_target_type_found = 0)
            [
    ...
    [
      ...
      [
        ...
        [ ; target found
          ...
          ifelse ((interaction_count > 0) and (interaction_change = FALSE))
          [
            set distance_to_best_neighbour 20 ; Add "attack" distance
            set received_distance_to_target_type_0 30
            set color ORANGE
            rt 180
            ; make received seq number unatractive to allow to move away
            set received_sequence_number_target_type_0 1  ; Add "attack" sequence number
            set interaction_change TRUE
          ]
          [
            ifelse ((interaction_count > 0) and (interaction_change = TRUE))
            [
            ...
            [
              ifelse ((interaction_count = 0) and (interaction_change = TRUE))
              [
                set distance_to_best_neighbour 1  ; Add "attack" distance
...
to check_North_East [x-offset y-offset possible_area colour_required]
    ...
    [
        ...
        [
          ...
          set x-offset x-offset + 1 ; does this push it out of the area after FALSE?
        ]
...
to place_independents [sub_type]
  ...
  create-independents (number_of_independents)
  [
    set step_size_scaling ((world-width - 2) / size_of_sides) ; -2 because of boundary. At begining to allow required scaling factors
    set color WHITE
    set size 1
...
531
File_Name
HUNTER RUNNER - FIXED - Simple_Maze
1
0
String
